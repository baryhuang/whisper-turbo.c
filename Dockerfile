...
RUN git clone https://github.com/ggml-org/whisper.cpp.git /reference && cd /reference && git checkout c44b60b8053bbf2a5c1e014f11323fb3f2485177 && cmake -S . -B build -DCMAKE_BUILD_TYPE=Release -DGGML_NATIVE=OFF -DGGML_CPU_ALL_VARIANTS=ON -DGGML_CUDA=OFF -DWHISPER_BUILD_TESTS=OFF -DWHISPER_BUILD_SERVER=OFF && cmake --build build -j8

FROM debian:bookworm-slim
RUN apt-get update && apt-get install -y --no-install-recommends libgomp1 libstdc++6 ca-certificates curl time util-linux && rm -rf /var/lib/apt/lists/*
WORKDIR /app
