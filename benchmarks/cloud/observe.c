...
int main(int argc,char **argv) {
    if(argc<3){fprintf(stderr,"usage: observe MODEL COMMAND [ARG...]\n");return 2;}
    int model=open(argv[1],O_RDONLY|O_CLOEXEC);struct stat st;
    if(model<0||fstat(model,&st)||st.st_size<=0)return 2;
    ...
    if(warm>1){fprintf(stderr,"model cache eviction incomplete: %zu resident pages\n",warm);return 2;}
    ...
    if(!child){execvp(argv[2],argv+2);perror("exec");_exit(127);}
    ...
    fprintf(stderr,"OBSERVATION {\"counter\":\"execution_cgroup_lifetime_peak\",\"prior_peak_bytes\":%llu,\"initial_current_bytes\":%llu,\"peak_bytes\":%llu,\"peak_rss_bytes\":%llu,\"seconds\":%.6f,\"exit_code\":%d,\"model_initial_resident_pages\":%zu,\"swap_configured\":false}\n",
        before,initial,after,(unsigned long long)usage.ru_maxrss*1024,seconds,code,warm);
    return code;
}
