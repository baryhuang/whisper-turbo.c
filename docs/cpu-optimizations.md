...
| --- | --- | --- |
| `WHISPER_ACTIVATIONS=int8` | Q8 weights and INT8 encoder activations | Existing opt-in |
| `WHISPER_DECODER_ACTIVATIONS=int8` | INT8 activation matrix/vector operations in decoder projections and vocabulary head | Off |
...
per thread at first use. INT8 decoder activations remain opt-in; this does not
quantize attention, normalization, or diarization weights to INT8.
